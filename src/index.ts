...
function todo(container: Element) {
  ...
  form.addEventListener("submit", (event: Event) => {
    event.preventDefault(); // prevent it from submitting the form when enter is pressed
    const input = form.elements["text"] as HTMLInputElement;
    // can also select using form.querySelector("input[name='text']");
    addItem(input.value);
    form.reset();
  });
  ...
  function addItem(itemText: string) {
    ...
    item.appendChild(check); // there is also prependChild if we want it to show above
